...
from api.myutils.const import NUMBER_TO_LECTURE_TIMES, PREPARE_TIME
...
class TimeslotJS(BaseModel):
    ...
    def to_timeslot(self) -> "Timeslot":
        assert (
            self.timeslot_type == "lecture"
        ), f"timeslot_type must be lecture, but {self.timeslot_type}"
    ...
    @classmethod
    def from_timeslot(cls, timeslot: "Timeslot") -> "TimeslotJS":
        return TimeslotJS(
            year=timeslot.start_time.year,
            month=timeslot.start_time.month,
            day=timeslot.day,
    ...
    def timeslot_number_range(self) -> Self:
        if self.timeslot_type == "lecture":
            if self.timeslot_number < 1:
                raise ValueError(
                    f"timeslot_number must be >= 1, but {self.timeslot_number}"
                )
        elif self.timeslot_type == "office_work":
            if self.timeslot_number != 0:
                raise ValueError(
                    f"timeslot_number must be 0, but {self.timeslot_number}"
                )
        return self
...
class MonthlyAttendanceBeforeCalculate(BaseModel):
    ...
    def update(self, req: UpdateAttendanceReq) -> "MonthlyAttendanceBeforeCalculate":
        new_lecture_time_slot_list = [
            timeslot_js.to_timeslot()
            for timeslot_js in req.timeslot_js_list
            if timeslot_js.timeslot_type == "lecture"
        ]
        office_time_slot_list = [
            timeslot
            for timeslot in self.timeslot_list
            if timeslot.timeslot_type == "office_work"
        ]
        time_slot_list = new_lecture_time_slot_list + office_time_slot_list
    ...
    @classmethod
    def from_monthly_attendance(
        cls, monthly_attendance: "MonthlyAttendance"
    ...
    def calc_salary(self, gensen: pd.DataFrame) -> "MonthlyAttendance":
        ...
        for timeslot in timeslot_list:
            if timeslot.timeslot_type == "lecture":
                ...
                raise ValueError(
                    f"timeslot_type must be lecture or office_work, but {timeslot.timeslot_type}"
                )
        ...
        for day in range(31):
            ...
            if len(daily_lectures) == 0:
                ...
                prepare_amount_min_after_end = (len(daily_lectures) + 1) * PREPARE_TIME
                prepare_amount_min_before_end = (len(daily_lectures) - 1) * PREPARE_TIME
            officework_amount_min += (
                prepare_amount_min_after_end + prepare_amount_min_before_end
            )
            ...
            # 8時間超勤務手当
            over_eight_hour_amount_min = max(
                0, lecture_amount_min + officework_amount_min - 8 * 60
            )
            daily_salary_day += (
        ...
        idx = (gensen["min"] <= monthly_gross_extra) & (
            monthly_gross_extra < gensen["max"]
        )
        assert np.sum(idx) == 1
...
class MonthlyAttendance(MonthlyAttendanceBeforeCalculate):
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, MonthlyAttendance):
    ...
    @classmethod
    def from_model(
        cls, monthly_attendance_model: MonthlyAttendanceModel
    ) -> "MonthlyAttendance":
        ...
        timeslot_list = [
            Timeslot(
                ...
                start_time=time_str_2_datetime(
                    year, month, int(timeslot.day), timeslot.start_time
                ),
                end_time=time_str_2_datetime(
                    year, month, int(timeslot.day), timeslot.end_time
                ),
        ...
        if monthly_attendance_model.remark is None:
